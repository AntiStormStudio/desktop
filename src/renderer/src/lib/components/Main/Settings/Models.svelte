<script lang="ts">
  ...
  let activeDownloads = $state<Map<string, { repo: string; filename: string; percent: number }>>(
    new Map()
  )
                ...
                {:else if dlActive}
                  <div class="flex items-center gap-1.5 shrink-0">
                    <span class="text-[10px] opacity-40 font-mono"
                      >{getDownloadPercent(selectedRepo, file.filename).toFixed(0)}%</span
                    >
                    <button
          ...
          <div class="flex flex-col max-h-[300px] overflow-y-auto">
            {#each searchResults as repo}
              ...
              >
                ...
                <svg
                  class="w-3 h-3 opacity-15 shrink-0"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  stroke-width="2"
