<script lang="ts">
  ...
  let lsInfo = $state<{ url?: string; status?: string; pid?: number; binaryPath?: string } | null>(
    null
  )
...
{:else if !installed}
  ...
  <div class="py-4 flex items-center justify-between border-t border-white/[0.04]">
    ...
    <input
      ...
      onchange={(e) =>
        updateConfig('version', (e.target as HTMLInputElement).value.trim() || 'latest')}
    />
  </div>
  ...
  <div class="py-4 flex items-center justify-between border-t border-white/[0.04]">
    ...
    >
      {#each variantOptions as opt}
        <option value={opt.value} selected={($config?.llamaCpp?.variant ?? 'auto') === opt.value}
          >{opt.label}</option
        >
      {/each}
    </select>
  ...
  <div class="flex flex-col divide-y divide-white/[0.04]">
    ...
    <div class="py-4">
      <div class="flex items-center justify-between mb-3">
        ...
        <div class="flex items-center gap-1.5">
          ...
          {:else if lsInfo?.status === 'starting' || lsInfo?.status === 'setting-up'}
            ...
            <span class="text-[12px] opacity-30 capitalize"
              >{lsInfo?.status === 'setting-up'
                ? $i18n.t('settings.inference.settingUp')
                : $i18n.t('common.starting')}</span
            >
      ...
      <div class="flex items-center gap-2">
        ...
        <button
          class="text-[12px] opacity-40 hover:opacity-70 px-3 py-1.5 bg-black/[0.04] dark:bg-white/[0.06] transition border-none text-[#1d1d1f] dark:text-[#fafafa] rounded-xl flex items-center gap-1.5 {settingUp
            ? 'pointer-events-none opacity-20'
            : ''}"
          disabled={settingUp}
          onclick={setupServer}
        >
          ...
          {:else}
            ...
            {lsInfo?.binaryPath
              ? $i18n.t('settings.inference.redownload')
    ...
    {#if isRunning && lsInfo}
      <div class="py-4">
        <div class="text-[13px] opacity-70 mb-3">
          {$i18n.t('settings.inference.runningInstance')}
        </div>
        <div class="flex flex-col gap-2">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              onclick={() => window.open(lsInfo.url)}>{lsInfo.url}</button
            >
          </div>
          <div class="flex items-center justify-between">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          {#if updateStatus === 'up-to-date'}
            <div class="text-[11px] opacity-25 mt-0.5">
              {$i18n.t('settings.inference.upToDate')}
            </div>
          {:else if updateStatus === 'available' && updateInfo?.latestVersion}
            <div class="text-[11px] opacity-40 mt-0.5">
              {$i18n.t('settings.inference.updateAvailable', { version: updateInfo.latestVersion })}
            </div>
          {:else if updateStatus === 'updating'}
            <div class="text-[11px] opacity-25 mt-0.5">
              {$i18n.t('settings.inference.updating')}
            </div>
        ...
        <div>
          ...
          {:else if updateStatus === 'updating'}
            ...
            >
              <div
                class="w-2.5 h-2.5 rounded-full border-2 border-black/20 dark:border-white/30 border-t-transparent animate-spin"
              ></div>
    ...
    <div class="py-4 flex items-center justify-between">
      <div>
        ...
        <div class="text-[11px] opacity-25 mt-0.5">
          {$i18n.t('settings.inference.startOnLaunchDesc')}
        </div>
      </div>
      <Switch
    ...
    <div class="py-4 flex items-center justify-between">
      ...
      <input
        ...
        onchange={(e) =>
          updateConfig('port', parseInt((e.target as HTMLInputElement).value) || 18881)}
      />
    </div>
    ...
    <div class="py-4 flex items-center justify-between gap-4">
      <div class="shrink-0">
        ...
        <div class="text-[11px] opacity-25 mt-0.5">
          {$i18n.t('settings.inference.extraArgumentsDesc')}
        </div>
      </div>
      <input
        type="text"
