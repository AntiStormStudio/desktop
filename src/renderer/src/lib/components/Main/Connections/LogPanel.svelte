...
>
  ...
  <div class="h-0 shrink-0 cursor-ns-resize relative" onmousedown={onDragStart}>
  ...
  <!-- svelte-ignore a11y_no_static_element_interactions -->
  <div
    class="flex items-center justify-between px-3 py-1 shrink-0 border-b border-white/[0.06] cursor-pointer"
    onclick={onClose}
  >
    <div class="flex items-center gap-2">
      <span class="text-[11px] uppercase tracking-wider text-white/40 font-medium"
        >{logLabels[activeLog]?.() ?? activeLog}</span
      >
    </div>
    <!-- svelte-ignore a11y_no_static_element_interactions -->
    <div class="flex items-center gap-0.5" onclick={(e) => e.stopPropagation()}>
      ...
      {#if onStop && serviceReady}
        <button
          class="p-1 rounded-md hover:bg-white/[0.08] transition bg-transparent border-none cursor-pointer {stopping
        ...
        >
          {#if stopping}
            <div
              class="w-3.5 h-3.5 rounded-full border-[1.5px] border-white/30 border-t-transparent animate-spin"
            ></div>
          {:else}
            <svg
              class="w-3.5 h-3.5"
    ...
    {:else}
      <div class="absolute inset-0 flex items-center justify-center bg-[#0a0a0a]">
        <div class="flex flex-col items-center gap-3">
          <div
            class="w-5 h-5 rounded-full border-2 border-white/15 border-t-white/50 animate-spin"
          ></div>
